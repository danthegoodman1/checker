#!/bin/bash
# Test Firecracker snapshot/restore functionality (using Rust trigger binary)
# Usage:
#   ./fc-snapshot-test-rust.sh create <dir_with_dockerfile> <kernel>  - Build image, boot VM, snapshot when ready
#   ./fc-snapshot-test-rust.sh restore <kernel>                       - Restore from snapshot and run
#
# How it works:
#   CREATE: Boot VM with init that prints "===SNAPSHOT_READY===" then runs fc-trigger
#           which polls /dev/vdb for magic byte 0x01. Trigger disk has 0x00, so it spins.
...
#            fc-trigger sees 0x01, runs /.fc_cmd, prints exit code, reboots.
...
FC_TRIGGER="$SCRIPT_DIR/fc-trigger/target/release/fc-trigger"

die() { echo "error: $1" >&2; exit 1; }

[[ -f "$FC_TRIGGER" ]] || die "fc-trigger binary not found. Run: $SCRIPT_DIR/fc-trigger/build.sh"
...
build_rootfs() {
    ...
    # Create command script (executed by fc-trigger after restore)
    cat > "$FS/.fc_cmd" <<EOF
#!/bin/sh
$ENV_VARS
cd $WORKDIR
exec $ENTRYPOINT $CMD_ARGS
EOF
    chmod +x "$FS/.fc_cmd"
    
    # Copy fc-trigger binary
    cp "$FC_TRIGGER" "$FS/fc-trigger"
    chmod +x "$FS/fc-trigger"
    
    # Generate minimal init
...
cmd_create() {
    ...
    echo ""
    echo "Test restore with: ./fc-snapshot-test-rust.sh restore $KERNEL"
}
