...
echo "=== Firecracker VM init starting ==="
...
echo "Checking for /etc/checker.env..."
if [ -f /etc/checker.env ]; then
    echo "Loading runtime environment from /etc/checker.env:"
    cat /etc/checker.env
    . /etc/checker.env
else
    echo "WARNING: /etc/checker.env not found!"
    echo "Contents of /etc:"
    ls -la /etc/ 2>&1 || echo "Failed to list /etc"
fi

# Debug: show key environment variables
echo "CHECKER_API_URL=\${CHECKER_API_URL:-NOT SET}"
echo "CHECKER_JOB_ID=\${CHECKER_JOB_ID:-NOT SET}"

# Change to working directory
cd $WORKDIR

# Run the container's entrypoint
echo "Starting: $ENTRYPOINT $CMD"
$ENTRYPOINT $CMD
EXIT_CODE=\$?
