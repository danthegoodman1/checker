...
// Runtime implements the runtime.Runtime interface for Docker on macOS.
// Since Docker checkpoint requires CRIU (Linux-only), checkpoint/restore operations
// on macOS will stop/start the container instead.
...
// dockerCheckpoint holds the data needed to restore a container on macOS.
// Since checkpoint isn't available, this stores enough to restart the container.
...
func (h *processHandle) String() string {
	return fmt.Sprintf("docker-darwin[exec=%s,container=%s]", h.executionID, h.containerID)
}
...
func (h *processHandle) Checkpoint(ctx context.Context, keepRunning bool) (runtime.Checkpoint, error) {
	h.logger.Debug().
		...
		Msg("checkpoint requested (using docker stop/start - CRIU not available on macOS)")
	...
	if !keepRunning {
		...
		h.logger.Debug().Msg("container stopped for checkpoint")
	}
...
func (r *Runtime) Restore(ctx context.Context, opts runtime.RestoreOptions) (runtime.Process, error) {
	...
	r.logger.Debug().
		...
		Msg("restoring container (using docker start - CRIU not available on macOS)")

	// Start the stopped container
	if err := r.client.ContainerStart(ctx, c.containerID, container.StartOptions{}); err != nil {
...
func (r *Runtime) startContainer(ctx context.Context, executionID string, env map[string]string, cfg *Config, apiHostAddress string, stdout, stderr io.Writer) (runtime.Process, error) {
	...
	// and use host.docker.internal which Docker Desktop provides on macOS
